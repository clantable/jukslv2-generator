...
func Gen(c config.Config, model string) {
	params := map[string]any{
		...
		"package":      strcase.ToSnake(c.DeliveryConfig.Package),
		"deliveryPath": strings.Split(c.DeliveryConfig.Path, ".")[1],
		"usecasePath":  strings.Split(c.UsecaseConfig.Path, ".")[1],
	}
	...
	// }
	if err := ioutil.WriteFile(c.DeliveryConfig.Path+"/"+model+".go", out.Bytes(), 0644); err != nil {
